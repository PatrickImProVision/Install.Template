...
<head>
    ...
    <title><?= esc($title ?? 'Contact CMS') ?></title>
    <style>
        body {
            margin: 0;
...
<body>
<header>
    <h1><?= esc($title ?? 'Contact blocks') ?></h1>
    <nav>
...
<main>
    ...
    <div class="top-actions">
        <a href="<?= esc(site_url('DashBoard/Site_Contact/Create')) ?>">+ New block</a>
    </div>

    <div class="panel">
        <p class="muted">Rows drive the site footer and the <strong>/contact</strong> page heading. Use <strong>page_heading</strong> for the contact hero; <strong>brand</strong> / column entries / <strong>legal</strong> match the footer layout.</p>

        <?php if ($items === []) : ?>
            <p>No blocks yet. <a href="<?= esc(site_url('DashBoard/Site_Contact/Create')) ?>">Create the first block</a>.</p>
        <?php else : ?>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php foreach ($items as $row) : ?>
                        <tr>
                            ...
                            <td><?= esc((string) ($row['column_group'] ?? '')) ?></td>
                            <td><?php
                                $kind = (string) ($row['kind'] ?? '');
                                $label = $kind === 'legal'
                                    ? (string) ($row['description'] ?? '')
                                    : (string) ($row['title'] ?? '');
                                echo esc(strlen($label) > 56 ? substr($label, 0, 53) . '…' : $label);
                            ?></td>
                            <td class="actions">
                                <a href="<?= esc(site_url('DashBoard/Site_Contact/Edit/' . (int) ($row['id'] ?? 0))) ?>">Edit</a>
                                <?= form_open(site_url('DashBoard/Site_Contact/Delete/' . (int) ($row['id'] ?? 0))) ?>
                                    <?= csrf_field() ?>
