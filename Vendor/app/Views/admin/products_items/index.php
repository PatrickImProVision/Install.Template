...
<head>
    ...
    <title><?= esc($title ?? 'Products CMS') ?></title>
    <style>
        body {
            margin: 0;
...
<body>
<header>
    <h1><?= esc($title ?? 'Products blocks') ?></h1>
    <nav>
...
<main>
    ...
    <div class="top-actions">
        <a href="<?= esc(site_url('DashBoard/Products/Create')) ?>">+ New block</a>
    </div>

    <div class="panel">
        <p class="muted">Each row is one block on the public Products &amp; services page. Use one <strong>page heading</strong> for the section title; add <strong>product cards</strong> for each offering (gradient style, link, icon SVG).</p>

        <?php if ($items === []) : ?>
            <p>No blocks yet. <a href="<?= esc(site_url('DashBoard/Products/Create')) ?>">Create the first block</a>.</p>
        <?php else : ?>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php foreach ($items as $row) : ?>
                        <tr>
                            ...
                            ?></td>
                            <td class="actions">
                                <a href="<?= esc(site_url('DashBoard/Products/Edit/' . (int) ($row['id'] ?? 0))) ?>">Edit</a>
                                <?= form_open(site_url('DashBoard/Products/Delete/' . (int) ($row['id'] ?? 0))) ?>
                                    <?= csrf_field() ?>
