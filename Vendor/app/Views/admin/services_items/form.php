...
<head>
    ...
    <title><?= esc($title ?? 'Services block') ?></title>
    <style>
        body {
            margin: 0;
...
<header>
    ...
    <nav>
        <a href="<?= esc(site_url('DashBoard/Services')) ?>">All blocks</a>
        &nbsp;·&nbsp;
...
<main>
    <div class="panel">
        <?= form_open($action ?? '') ?>
            ...
            <label for="kind">Kind <span class="hint">page_heading = section title &amp; subtitle; service_card = one offering card</span></label>
            <select name="kind" id="kind">
                <?php foreach (\App\Controllers\Admin\ServicesItems::KINDS as $k) : ?>
            ...
            <label for="description">Description <span class="hint">Section subtitle (heading) or card intro paragraph</span></label>
            ...
            <label for="bullets">Bullets <span class="hint">Service cards only — one line per bullet</span></label>
            ...
            <label for="image_url">Image URL <span class="hint">Service cards — hero image</span></label>
            <input type="text" name="image_url" id="image_url" maxlength="1024" value="<?= esc(old('image_url', (string) ($record['image_url'] ?? ''))) ?>" placeholder="https://">
            <?php if (! empty($errors['image_url'])) : ?><div class="err"><?= esc($errors['image_url']) ?></div><?php endif; ?>

            <label for="image_alt">Image alt text</label>
            <input type="text" name="image_alt" id="image_alt" maxlength="255" value="<?= esc(old('image_alt', (string) ($record['image_alt'] ?? ''))) ?>">
            <?php if (! empty($errors['image_alt'])) : ?><div class="err"><?= esc($errors['image_alt']) ?></div><?php endif; ?>

            <label for="icon_svg">Icon SVG <span class="hint">Service cards — paste full &lt;svg&gt;…&lt;/svg&gt; for the floating icon</span></label>
            ...
            <div class="actions">
                ...
                <a href="<?= esc(site_url('DashBoard/Services')) ?>">Cancel</a>
            </div>
        <?= form_close() ?>
    </div>
</main>
