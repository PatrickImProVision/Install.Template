...
<body>
<header>
    <h1><?= esc($title ?? 'Services blocks') ?></h1>
    <nav>
...
<main>
    ...
    <div class="top-actions">
        <a href="<?= esc(site_url('DashBoard/Services/Create')) ?>">+ New block</a>
    </div>

    <div class="panel">
        <p class="muted">Each row is one block on the public Services page. Use one <strong>page heading</strong> for the section title; add <strong>service cards</strong> for each offering.</p>

        <?php if ($items === []) : ?>
            <p>No blocks yet. <a href="<?= esc(site_url('DashBoard/Services/Create')) ?>">Create the first block</a>.</p>
        <?php else : ?>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php foreach ($items as $row) : ?>
                        <tr>
                            ...
                            ?></td>
                            <td class="actions">
                                <a href="<?= esc(site_url('DashBoard/Services/Edit/' . (int) ($row['id'] ?? 0))) ?>">Edit</a>
                                <?= form_open(site_url('DashBoard/Services/Delete/' . (int) ($row['id'] ?? 0))) ?>
                                    <?= csrf_field() ?>
