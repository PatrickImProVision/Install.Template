...
<head>
    ...
    <title><?= esc($title ?? 'Our values CMS') ?></title>
    <style>
        body {
            margin: 0;
...
<body>
<header>
    <h1><?= esc($title ?? 'Our values blocks') ?></h1>
    <nav>
...
<main>
    ...
    <div class="top-actions">
        <a href="<?= esc(site_url('DashBoard/Values/Create')) ?>">+ New block</a>
    </div>

    <div class="panel">
        <p class="muted">Each row is one block on the public Our values page. Use one <strong>page heading</strong> for the section title; add <strong>value items</strong> for each principle (optional emoji + text).</p>

        <?php if ($items === []) : ?>
            <p>No blocks yet. <a href="<?= esc(site_url('DashBoard/Values/Create')) ?>">Create the first block</a>.</p>
        <?php else : ?>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php foreach ($items as $row) : ?>
                        <tr>
                            ...
                            ?></td>
                            <td class="actions">
                                <a href="<?= esc(site_url('DashBoard/Values/Edit/' . (int) ($row['id'] ?? 0))) ?>">Edit</a>
                                <?= form_open(site_url('DashBoard/Values/Delete/' . (int) ($row['id'] ?? 0))) ?>
                                    <?= csrf_field() ?>
