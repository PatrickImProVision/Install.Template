-- Products & services page CMS (PostgreSQL).
...
INSERT INTO products_items (id, sort_order, kind, title, description, bullets, sub_line, href, card_style, icon_svg, created_at, updated_at) VALUES
(1, 0, 'page_heading', 'Products & services', 'We build innovative products and deliver hosting at a high standard', NULL, NULL, NULL, NULL, NULL, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
...
(5, 40, 'product_card', 'Tic-tac-toe (Gomoku-style) →', 'An interactive Discord bot for tic-tac-toe style play with single-player and multi-player modes and a custom language pack.', E'Single & multi-player\nCustom language pack', 'tictactoe-bot.xyz', 'https://tictactoe-bot.xyz/docs', 'grad-purple', $$<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M3 9h18"/><path d="M3 15h18"/><path d="M9 3v18"/><path d="M15 3v18"/></svg>$$, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
ON CONFLICT (id) DO NOTHING;

SELECT setval(
    pg_get_serial_sequence('products_items', 'id'),
    COALESCE((SELECT MAX(id) FROM products_items), 1)
);
