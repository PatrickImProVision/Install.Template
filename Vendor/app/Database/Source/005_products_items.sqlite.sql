-- Products & services page CMS (SQLite 3).

CREATE TABLE IF NOT EXISTS products_items (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  sort_order INTEGER NOT NULL DEFAULT 0,
  kind TEXT NOT NULL DEFAULT 'product_card',
...
CREATE INDEX IF NOT EXISTS products_items_sort_idx ON products_items (sort_order);

INSERT OR IGNORE INTO products_items (id, sort_order, kind, title, description, bullets, sub_line, href, card_style, icon_svg, created_at, updated_at) VALUES
(1, 0, 'page_heading', 'Products & services', 'We build innovative products and deliver hosting at a high standard', NULL, NULL, NULL, NULL, NULL, datetime('now'), datetime('now')),
...
Custom language pack', 'tictactoe-bot.xyz', 'https://tictactoe-bot.xyz/docs', 'grad-purple', '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M3 9h18"/><path d="M3 15h18"/><path d="M9 3v18"/><path d="M15 3v18"/></svg>', datetime('now'), datetime('now'));
